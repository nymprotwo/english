...
const Storage = {
  ...
  restore() {
    try {
      ...
      // Recovery: if DB.currentWorkout is null but sb_current has sets,
      // reconstruct it so finishWorkout() can save the workout to history.
      // This happens after a page reload mid-workout (OAuth redirect, etc.)
      if (!DB.currentWorkout) {
        const hasAnySets = data.workout.some(ex => ex.sets && ex.sets.length > 0);
        if (hasAnySets) {
          DB.currentWorkout = {
            id:        Date.now(),
            date:      data.savedDate,
            ...
            exercises: data.workout.map(ex => ({
              ...
              accent:  ex.accent,
              grip:    ex.gripType,
              sets:    (ex.sets || []).map(s => ({ ...s, ts: s.ts || Date.now() })),
              finished: false,
            })),
            finished: false,
          };
