...
const App = {
  ...
  render() {
    ...
    this.renderModal();
    // Auto-save active training (including plan/build drafts)
    if (['active','regress_check','plan','build'].includes(State.diary.stage)) Storage.saveTraining();
  },
...
const Diary = {
  ...
  buildWorkout() {
    ...
    // DB layer: create workout record and register all exercises
    startWorkout();
  ...
  addToPlan() {
    ...
    if (categories) {
      ...
      effectiveCatId = sel.categoryId || (categories.length === 1 ? categories[0].id : null);
    ...
    State.diary.plan.push({
      exId, name: exData.name, muscle: sel.muscle, equip, accent,
      gripType: 'straight', gripWidth: 'medium', stance: 'shoulder',
      tut: 'normal', rest: '120', tutExpanded: false, restExpanded: false,
      typeRef: { catId: effectiveCatId, typeId: sel.typeId },
    });
  ...
  // ── Plan Detail (Screen 2) ───────────────────────────
  openPlanDetail(idx) {
    State.diary.planDetailIdx = idx;
    App.render();
  },
  closePlanDetail() {
    State.diary.planDetailIdx = null;
    App.render();
  },
  // Установить угол скамьи в активном упражнении (setup screen)
  setSetupAngle(angle) {
    const ex = State.diary.workout[State.diary.activeIdx];
    if (!ex) return;
    if (typeof CHEST_ANGLE_MAP !== 'undefined' && CHEST_ANGLE_MAP[ex.equip]) {
      const a = CHEST_ANGLE_MAP[ex.equip][angle];
      if (a) { ex.id = a.exId; ex.name = a.name; }
    }
    ex.accent = angle;
    App.render();
  },

  // Установить угол скамьи (акцент) в плановом упражнении; обновляет exId + name
  setPlanAngle(angle) {
    const idx = State.diary.planDetailIdx;
    if (idx === null || !State.diary.plan[idx]) return;
    const p = State.diary.plan[idx];
    const amap = (typeof CHEST_ANGLE_MAP !== 'undefined') ? (CHEST_ANGLE_MAP[p.equip] || {}) : {};
    const a = amap[angle];
    if (a) { p.exId = a.exId; p.name = a.name; }
    p.accent = angle;
    App.render();
  },
  // Установить произвольное поле планового упражнения (gripWidth, tut, rest, ...)
  setPlanField(field, value) {
    const idx = State.diary.planDetailIdx;
    if (idx === null || !State.diary.plan[idx]) return;
    State.diary.plan[idx][field] = value;
    App.render();
  },
  // Сохранить конфигурацию планового упражнения в «Любимые упражнения»
  saveFavEx() {
    const idx = State.diary.planDetailIdx;
    if (idx === null || !State.diary.plan[idx]) return;
    const p = State.diary.plan[idx];
    const favs = JSON.parse(localStorage.getItem('sb_fav_ex') || '[]');
    const entry = { exId:p.exId, name:p.name, muscle:p.muscle, equip:p.equip,
                    accent:p.accent, gripWidth:p.gripWidth, tut:p.tut, rest:p.rest };
    // Дедупликация по exId+equip+accent+gripWidth
    const key = [entry.exId, entry.equip, entry.accent||'', entry.gripWidth].join('|');
    const exists = favs.findIndex(f => [f.exId,f.equip,f.accent||'',f.gripWidth].join('|') === key);
    if (exists !== -1) favs.splice(exists, 1);
    favs.unshift(entry);
    if (favs.length > 12) favs.pop();
    localStorage.setItem('sb_fav_ex', JSON.stringify(favs));
    App.render();
  },
  ...
  startWorkout() {
    const blocks = State.diary.plan.map(p => ({
      ...
      equip: p.equip, gripType: p.gripType || 'straight', gripWidth: p.gripWidth || 'medium', stance: p.stance || 'shoulder', tut: p.tut || 'normal', rest: p.rest || '120',
      accent: p.accent,
      equipChosen: true, gripChosen: true, setupDone: false, tutExpanded: false, restExpanded: false, variationExpanded: false, variation: null,
      sets: [], note: '',
    }));
