...
Screens.workoutActive = function() {
    ...
    const showGripType  = GRIP_TYPE_EQUIPS.has(ex.equip) && !showStance && !PRESS_EX_IDS.has(ex.id) && !NO_GRIP_UI_EX_IDS.has(ex.id);
    const showGripWidth = GRIP_WIDTH_EQUIPS.has(ex.equip) && !showStance && !NO_GRIP_UI_EX_IDS.has(ex.id);
    ...
    if (!ex.setupDone) {
      ...
      html += '<span class="lbl">' + ex.muscle.toUpperCase() + '</span>';
      ...
      // ── Угол скамьи (для грудных упражнений) ──────────────
      const _BENCH_ANGLE_OPTS = {
        barbell:      [{id:'upper',icon:'↗',label:'Наклон',sub:'30–45°'},{id:'middle',icon:'→',label:'Горизонт.',sub:'Середина'},{id:'lower',icon:'↘',label:'Декл.',sub:'Низ груди'}],
        dumbbells:    [{id:'upper',icon:'↗',label:'Наклон',sub:'30–45°'},{id:'middle',icon:'→',label:'Горизонт.',sub:'Середина'},{id:'lower',icon:'↘',label:'Декл.',sub:'Низ груди'}],
        smith:        [{id:'upper',icon:'↗',label:'Наклон',sub:'30–45°'},{id:'middle',icon:'→',label:'Горизонт.',sub:'Середина'},{id:'lower',icon:'↘',label:'Декл.',sub:'Низ груди'}],
        hammer:       [{id:'upper',icon:'↗',label:'Наклон',sub:'Верх груди'},{id:'middle',icon:'→',label:'Горизонт.',sub:'Середина'}],
        cable_single: [{id:'upper',icon:'↗',label:'Верх',sub:'Блок снизу'},{id:'middle',icon:'→',label:'Середина',sub:'Прямой'},{id:'lower',icon:'↘',label:'Низ',sub:'Блок сверху'}],
        bodyweight:   [{id:'upper',icon:'↗',label:'Ноги выше',sub:'Верх груди'},{id:'middle',icon:'→',label:'От пола',sub:'Стандарт'}],
        dip_bars:     [{id:'upper',icon:'↗',label:'Вперёд',sub:'Акцент грудь'},{id:'middle',icon:'→',label:'Вертик.',sub:'Трицепс+грудь'}],
      };
      const _setupAngleOpts = (ex.muscle === 'Грудь' && typeof CHEST_ANGLE_MAP !== 'undefined' && CHEST_ANGLE_MAP[ex.equip])
        ? (_BENCH_ANGLE_OPTS[ex.equip] || null) : null;
      if (_setupAngleOpts) {
        html += '<div style="margin-bottom:16px">';
        html += '<div style="font-size:9px;color:var(--sb);letter-spacing:3px;margin-bottom:10px">УГОЛ СКАМЬИ</div>';
        html += '<div style="display:flex;gap:8px">';
        const _activeAngle = ex.accent || 'middle';
        _setupAngleOpts.forEach(a => {
          const on = _activeAngle === a.id;
          html += '<div onclick="Diary.setSetupAngle(\'' + a.id + '\')" style="flex:1;border-radius:12px;border:1.5px solid ' + (on?'var(--ac)':'var(--br)') + ';background:' + (on?'var(--ad)':'var(--sf)') + ';padding:12px 6px;text-align:center;cursor:pointer;transition:all .15s">';
          html += '<div style="font-size:20px;margin-bottom:5px;line-height:1;color:' + (on?'var(--ac)':'var(--mt)') + '">' + a.icon + '</div>';
    ...
    if (focusOn) {
      html += '<div style="margin-top:4px"><span style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:8px;background:var(--ac10);border:1px solid var(--ac25);font-size:9px;font-weight:700;color:var(--ac);letter-spacing:1px">⚡ ФОКУС</span></div>';
    } else {
    ...
    html += '</div>';
    if (!focusOn) {
      // ── Фото-квадрат (заглушка — в будущем фото упражнения) ──
      html += '<div style="width:80px;height:80px;border-radius:14px;background:#111316;border:1px solid #252530;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px">';
      html += '<span style="font-size:34px">🏋️</span>';
      html += '</div>';
    }
    html += '</div>';
