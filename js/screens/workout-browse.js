...
Screens.workoutBrowse = function() {
    ...
    // Draft = exercises chosen but zero sets logged (plan built, not started)
    const hasDraft = !hasOngoing && !!(State.diary.workout && State.diary.workout.length > 0);
    ...
    if (hasOngoing) {
      // Незавершённая тренировка — кнопка "Продолжить" + быстрое завершение
      const _onWo   = State.diary.workout;
      const _onSets = _onWo.reduce((s, ex) => s + ex.sets.filter(st => st.type === 'work').length, 0);
      const _onTon  = _onWo.reduce((s, ex) => s + ex.sets.filter(st => st.type === 'work').reduce((s2, st) => s2 + (st.weight||0) * (st.reps||0), 0), 0);
      const _tfmt   = t => t >= 1000 ? (t/1000).toFixed(1) + ' т' : t + ' кг';
      ...
      // Строка со статистикой + быстрое завершение
      html += '<div style="background:var(--sf);border:1px solid var(--ac20);border-radius:12px;padding:11px 14px;display:flex;align-items:center;gap:10px">';
      html += '<div style="flex:1">';
      html += '<div style="font-size:9px;color:var(--ac);font-weight:700;letter-spacing:1px;margin-bottom:2px">ТЕКУЩАЯ ТРЕНИРОВКА</div>';
      html += '<div style="font-size:11px;color:var(--sb)">' + _onWo.length + ' упр · ' + _onSets + ' подх' + (_onTon > 0 ? ' · ' + _tfmt(_onTon) : '') + '</div>';
      html += '</div>';
      html += '<button onclick="Diary.finish()" style="padding:8px 14px;border-radius:9px;border:1px solid var(--ac30);background:var(--ac12);color:var(--ac);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap">Завершить ›</button>';
      html += '</div>';
      html += '</div>';
    ...
    // ── Draft card: exercises selected but no sets yet ──
    if (hasDraft) {
      const draftCount = State.diary.workout.length;
      const draftNames = State.diary.workout.slice(0, 2).map(e => e.name).join(', ') + (draftCount > 2 ? '...' : '');
      html += '<div style="margin-top:12px;background:var(--sf);border:1px solid var(--ac30);border-radius:13px;padding:13px 15px;display:flex;align-items:center;gap:12px">';
      html += '<div style="flex:1">';
      html += '<div style="font-size:11px;color:var(--ac);font-weight:700;letter-spacing:1px;margin-bottom:2px">НЕЗАВЕРШЁННЫЙ ПЛАН</div>';
      html += '<div style="font-size:12px;color:var(--sb)">' + draftCount + ' упр · ' + draftNames + '</div>';
      html += '</div>';
      html += '<button onclick="State.diary.workout=[];State.diary.plan=[];State.diary.activeIdx=-1;Storage.clearCurrent();App.render()" style="padding:7px 10px;border-radius:8px;border:1px solid var(--br);background:transparent;color:var(--sb);font-size:11px;cursor:pointer;font-family:inherit;margin-right:6px">Удалить</button>';
      html += '<button onclick="State.diary.stage=\'active\';App.render()" style="padding:7px 12px;border-radius:8px;border:none;background:var(--ad);color:var(--ac);font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">Открыть</button>';
      html += '</div>';
    }
